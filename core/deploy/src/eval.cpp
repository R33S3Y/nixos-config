...
eval::eval(const init &i) {
  ...
  for (string item : list) {
    ...
    if (item == "inputs")
      continue;
    resolveMap.insert({item,
                       {"nix eval " + flakePath + "#nixosConfigurations." +
  ...
  string eval::path(string test) {
    ...
    for (auto &entry :
         std::filesystem::directory_iterator(absoluteFolderPath)) {
  ...
  eval::result eval::attrsetKey(string test, bool canThrow) {
    ...
    if (resolveMap.count(attrsetKeys[0])) {
      cmd = resolveMap[attrsetKeys[0]][0] + attrset +
            resolveMap[attrsetKeys[0]][1];
    }
